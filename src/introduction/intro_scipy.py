...
def get_spectrum(signal_data: np.ndarray, sample_rate: float) -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    """
    # Calcular la FFT usando numpy (disponible)
    fft_result = np.fft.fft(signal)
    
    # Calcular las magnitudes (valor absoluto)
    ...
    # Tomar solo la parte positiva del espectro (hasta la frecuencia de Nyquist)
    positive_freq_mask = frecuencias >= 0
    frecuencias_positivas = frecuencias[positive_freq_mask] 
    magnitudes_positivas = magnitudes[positive_freq_mask]
    spectrum = (frecuencias, magnitudes_positivas)
    return spectrum
